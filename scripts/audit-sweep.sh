...
# with the newest earlier file. A probe whose tool is not installed records
# null, so a missing tool never reads as a clean result.
#
...
matches() {
  ...
  [ -d "$dir" ] || { echo 0; return; }
  { grep -rEo --include="$glob" --exclude-dir=build --exclude-dir="$exclude" -- "$pattern" "$dir" 2>/dev/null || true; } |
    wc -l | tr -d ' '
}
...
todo="$({ grep -rEn --include='*.rs' --include='*.swift' --exclude-dir=build 'TODO|FIXME|HACK' crates "$app" 2>/dev/null || true; } |
  { grep -vE '\(#[0-9]+\)' || true; } | wc -l | tr -d ' ')"
put simplicity.todo_without_issue "$todo"

# ── Principles ──
# Inline test modules sit at the foot of each file, so counting stops at the
# first #[cfg(test)] rather than parsing the module tree.
unwraps=0
if [ -d "$core" ]; then
  unwraps="$(find "$core" -type f -name '*.rs' -not -name 'tests.rs' -not -path '*/tests/*' \
    -exec awk '/#\[cfg\(test\)\]/ { nextfile } { n += gsub(/\.unwrap\(\)/, "") } END { print n + 0 }' {} + |
...
for d in ios/IntradaTests ios/IntradaUITests; do
  swift_tests=$((swift_tests + $(matches '@Test|func test[A-Z_]' "$d" '*.swift')))
done
...
{ [ -d "$app" ] && grep -rhoE --include='*.swift' --exclude-dir=build 'https?://[A-Za-z0-9.-]+' "$app" 2>/dev/null || true; } |
  LC_ALL=C sort -u | list network_hosts
put security.network_hosts "$(jq length "$tmp/list-network_hosts.json")"
leaks=null
...
if command -v cargo-deny >/dev/null 2>&1 && [ -f Cargo.lock ]; then
  cargo deny --offline --log-level error check advisories >"$tmp/deny.txt" 2>&1 || true
...
put activity.commits_since_last "$(git rev-list --count --since="$since" HEAD)"
{ git log --since="$since" --name-only --pretty=format: -- crates "$app" 2>/dev/null || true; } |
...
jq -r --slurpfile prev "$previous" '
  ...
  | "| \(.key) | \($was[.key] // "none") | \(.value // "skipped") |"' "$out"
echo
jq -r --slurpfile prev "$previous" '
