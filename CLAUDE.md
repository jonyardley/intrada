...
| `ios/Intrada/Generated/CodableExtensions.swift` | JSON Codable for API-facing types (hand-maintained) |
| `ios/Intrada/Generated/UniFFI/` | UniFFI bindings (`CoreFfi` class) |

**Two serialization paths**:
- **BCS (bincode)**: Used for all Crux FFI traffic — Events, Effects, ViewModel, Request/Resolve.
  Auto-generated by typegen. The iOS shell uses `CoreFfi` (Bridge pattern) for this.
- **JSON (Codable)**: Used only for REST API requests/responses. `CodableExtensions.swift`
  adds Codable conformance to the subset of types the API returns (Item, PracticeSession,
  Routine, Goal, and their sub-types). Uses `JSONEncoder.api`/`JSONDecoder.api` with
  `.convertToSnakeCase`/`.convertFromSnakeCase` key strategy.
