...
| **3b · iPad SplitView** | **Done.** New `LibrarySplitView` — size-class-driven: **compact (iPhone) is the unchanged `NavigationStack` push** (snapshots + flow untouched); **regular (iPad)** shows the Library list as a 380pt sidebar + a detail pane, selection-driven (`LibraryScreen(selection:)`), related items push within the pane. (The custom `ScrollView` Library doesn't map onto `NavigationSplitView`'s `List(selection:)` model, so a manual split keeps the compact path byte-identical.) iPad path is verified in-sim (snapshots pin iPhone13 only). | 2, 3 |
