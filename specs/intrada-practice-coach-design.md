...
meanwhile are specified in "Measures without machine listening". v7 (4 Aug
2026): user-created items and the built session (decision 19) — decision 18
dissolved decision 13's conflation of *unmeasurable* with *unauthored*, so
...
13. **Targets the graph cannot express are accepted, not forced** (added 3 Aug 2026). A declared campaign may contain items that map to no node — a teacher's "make the bridge sing", "sort out your time in the last A". These are kept as opaque targets: prescribable, self-confirmed rather than scored (the deploy-gate stance, decision 3), and never crammed into an ill-fitting node to preserve the appearance of coverage. Recurring unmatched targets are the authoring queue — they say which node to write next on evidence of real demand rather than taxonomy completionism (challenge 3). **Rescoped 4 Aug 2026 (decision 19):** this decision now governs only the *genuinely unmeasurable* unmatched target. As written it silently swallowed a second, different case — the countable-but-unauthored exercise — which decision 19 promotes to a first-class user node. The opaque-target treatment itself is unchanged.
...
5. **A campaign holds a *set* of targets, and the canonical source is a
   ...
   a node or pipeline stage where it can, and resolves the rest three ways
   (decision 19): a user drill on the matched node, a user node where the target
   is countable but unauthored, an opaque target (decision 13) only where it is
   genuinely unmeasurable. Then it back-chains and interleaves across the whole
   set. Nothing
...
### User-created items and the built session (added 4 Aug 2026, decision 19)

The scenario that forced this: a lesson hands over a new piece and a set of
exercises. The piece has a home (the tune pipeline — tunes are user-added by
nature). The exercises mostly don't: unless one happens to match an authored
node, v6 demoted it to an opaque target — prescribable but judgement-only, no
gate, no mastery, no cold test. For a user whose teacher is their main source
of material, that means most of what they actually practise earns nothing in
the model. The fix is decision 19's three-way resolution; this section is its
working detail.

- **A user node is a `Skill` with `origin: user`, not a new type.** No
  prerequisites (it can't unlock anything and nothing gates on it), a default
  low-band prior, gate criteria written at creation in the same schema as
  authored gates — which the countable-criterion framing makes cheap: the app
  asks for the same facts it already controls (tempo, scope, keys, passes).
  Creation is one short form or one dictated sentence, LLM-parsed into the
  criteria schema per decision 12, shown for confirmation, never silently
  saved.
- **The *serves* tag is display and tally only.** A user node may name the
  circle it feeds (head/hands) or an authored node it supports. The tag routes
  its time into the two-circle tally and groups it in the ability view; it
  never satisfies a prerequisite and never feeds the named node's mastery —
  that stronger link is exactly what tier (a) matching is for, with the user's
  explicit confirmation.
- **Matching is proposed, never assumed.** The LLM suggests "this looks like a
  shells exercise — track it under shells?" (decision 12: interpretation, not
  planning). Confirmed → user drill, evidence feeds the node at full weight,
  source-tagged. Declined or no candidate → user node. The user's confirmation
  is what earns the stronger coupling.
- **The planner treats user items as ordinary candidates.** Once created they
  are due, spaced, cold-tested and interleaved like authored content — which is
  the actual payoff: the teacher's exercise gets a schedule, not a shelf.
- **The built session is composition, not a mode.** From the steer surface, the
  user may assemble today's blocks from pipelines, nodes and their own items.
  Each block keeps its gate; evidence lands as if prescribed. The template
  shape is offered as advice and declinable (decision 19); the plan resumes
  tomorrow with the built session's evidence folded in, no reconciliation
  step. Repeated built sessions with a stable shape are themselves a signal —
...
rationale before the build. Chunks follow the tune's structure rather than a
bar count: the block opens on a whole section, a fumble splits it at the
nearest phrase mark (a stubborn corner can shrink to two bars), and clean
passes merge upward with each join taken as its own rung, until the full form
plays from memory, still out of time. Nothing asks what size to use; the size
finds itself. Only then does the click arrive, below the gate's target tempo, stepping up on
...
3. **The content burden.** Nodes x drills x criteria x method packs x cues x micro-lessons is a textbook's worth of authored material, validated against one learner. "The moat" is also a euphemism for hundreds of authoring hours. Mitigation: the minimal-content decision (5 nodes, 2 packs, 1 phrase for v1); author breadth only after the loop is proven — and let the unmatched-target queue (decision 13) choose the authoring order, so content follows demonstrated demand instead of filling in the taxonomy. Decision 19 sharpens the queue's signal and lowers the pressure at once: a countable target no longer *needs* authoring to be usable (it lives as a user node meanwhile), and a user node that earns weeks of use is the strongest authoring evidence there is — promotion, not speculation. One known hole jumps that queue on merit: **rhythm has no node.** It appears only as a modifier inside other things — "rhythmic constraints", "rhythmic displacement" — which is a strange omission for a design whose foundational claim is that time is the foundation. Time and feel are measured thoroughly and taught nowhere. A subdivision / displacement / rhythmic-vocabulary family is first in the authoring queue, ahead of a fifth voicing variant.
...
6. **Rigidity vs the texture of practice.** Practice has moods, and a plan that cannot bend gets abandoned rather than adjusted. Mitigation, in ascending order of commitment: today's steer (one tap, re-weaves the session around your choice), the built session (compose it yourself, everything still gates and counts — decision 19), re-scoping the campaign, off-piste (no plan, still logged), and the graduation arc (principle 8). Structure as default, never as cage — and never as a mode switch (decision 11).
...
**Phase 2b: steer and guard (3-4 weeks).** Everything that makes the loop kind rather than merely correct. The three declaration surfaces (goal, campaign, today's steer) with their defaults, so the app is usable having declared nothing; multi-target campaigns with the three-way target resolution (decision 19) — user drills onto matched nodes, user nodes for countable-unmatched targets, opaque targets (decision 13) for the genuinely unmeasurable; the built session as the steer surface's strongest form; back-chaining with honest horizons and the structural gap read; **the circling check** and the grind trade; the off-piano queue with its own cue (not blocks inside a piano session); unmonitored play alongside off-piste; the time-by-circle tally (arithmetic over already-tagged blocks, pulled forward from Phase 4); the judgement track — completion states, retirement and feel trends per §3 of the engine spec, its eventual *weighting* deferred with the divergence log (decision 17 as amended). Exit criterion: two weeks of daily use without reverting to self-directed sessions — where a *built* session now counts as use, not reversion, since it gates and tracks like everything else.
