...
| Every harness change gets a dated row in [`harness-log.md`](harness-log.md) naming the number it should move. `just usage 7 --quality` is read against the log every Monday, and a lever that has not moved its number after two reads is reverted, not patched | The log and the Monday read; the 2026-09-27 `just usage 14` read stays as the first fortnight check for #1849 | Fifteen harness PRs merged on 2026-09-14 with nothing saying what each should move, and the freeze set the same day held the harness still instead of measuring it (#1897) |
