...
| 2026-09-24 | #PRNUM (#2113) | The Mac gate runs on a push to main only when native paths changed, and a tree the runner has already seen pass skips the unit and UI tests there, on a PR or on main | Speed: claim to merged PR; the wait for the Mac runner on a PR, median 78 s and 529 s at the 90th percentile over 23 to 24 September (not in the report) | 2026-09-28 | |
| 2026-09-24 | #PRNUM (#2114) | `/ship` stops running the full iOS tier locally: the fast tier plus any UI class the diff edits, with CI's gate on the draft as the first full UI run. Takes #1887's row from once per PR to none | Speed: claim to merged PR; local gate time per PR, 4 to 5.5 minutes for the full tier in test-runner reports (not in the report) | 2026-09-28 | |
