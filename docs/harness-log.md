...
> replacing the freeze on harness work that ran from the evening before.
...
1. A harness change, in the repo or on Jon's machine, adds its row here in the
   ...
   next harness PR.
2. Every Monday, `just usage 7 --quality` is read against the table below, and
   each row whose first read has come gets what its number did in the Reads
   cell.
3. A lever whose number has not moved after two reads is reverted, not patched.
   A row whose number is "none" is not a lever and is never reverted by this
   rule. A number marked "not in the report" was scanned once from the
   transcripts; a read notes it as unreadable, not flat.
...
| Rework | Rung switches per session | `just usage 7 --quality`, the `sw` column | 5 of 125 sessions switched, four by hand (#1893) |
...
| Spend | Total and subagent share | `just usage 7`, the first line | $1,225 over 12 to 14 September, subagents 43% (#1849) |
...
| Spend | Long subagent runs and gate runs inside them | `just usage 7 --quality`, "Subagent runs by type" | The five biggest `task` runs $246 of $557; 264 gate runs across 44 `task` runs (#1893) |

## Changes

The first read for every row below is Monday 2026-09-21. Machine-local means a
file under `~/.claude/` on Jon's machine, dated by the PR that documented it.
...
| 2026-09-14 | #1843 (#1839) | Two streams by default; "what's next" answered from `just status` alone | Spend: status-only sessions, 12 of 76 (not in the report) | 2026-09-21 | |
...
| 2026-09-14 | #1887 (#1884) | The full iOS gate runs once per PR, not after every review fix | Speed: claim to merged PR; the gate's wall time is tracked on #1884 | 2026-09-21 | |
...
| 2026-09-15 | #1896 (#1891) | A screen change shows Jon its screenshot before the PR opens | Rework: reverts | 2026-09-21 | |
| 2026-09-15 | #1895 | `just usage` forwards extra flags, so `--quality` runs through the recipe | None: the measure itself | 2026-09-21 | |
| 2026-09-15 | #1897 | The harness words in the glossary; this log and the Monday read replace the freeze | None: the measure itself | 2026-09-21 | |
