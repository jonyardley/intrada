...
- [#1364](https://github.com/jonyardley/intrada/issues/1364) — live view:
  overall session timer (placement to design first). **Shipped and closed
  2026-09-02** in #1502, designed and specced with the Phase 4 round below.

### Phase 4 — Practice instruments (spec-first, Tier 3) — CLOSED 2026-09-03

- [#1366](https://github.com/jonyardley/intrada/issues/1366) — metronome;
  tempo becomes a tracked unit of measure. **Shipped and closed 2026-09-02**:
  the click in #1401, tempo capture in #1406, the tempo trend in #1422 /
  #1423 / #1425. What it did not cover — choosing which beats the click
  sounds on, and the time signature that makes that meaningful — is
  [#1499](https://github.com/jonyardley/intrada/issues/1499).
- [#1367](https://github.com/jonyardley/intrada/issues/1367) — pass counter,
  0 to 10, +1 perfect / -1 wrong, every increment recorded. **Shipped and
  closed 2026-09-03**: the core in #1505, the resident counter and its
  `Passes` / `Got it` / `Not quite` copy in #1506.
- [#1355](https://github.com/jonyardley/intrada/issues/1355) — photo
  aide-memoire on items; iteration 1 is exactly one photo. **Shipped and
  closed**, and grew a spec of its own along the way,
  [`specs/piece-from-photo.md`](../specs/piece-from-photo.md): phases A to C
  (a photo on a piece, reading the page into the add form, on-device model
  suggestions) are in v0.9.0. Phase D, a chord chart from a photo, is not
  started.

**Decision (Jon, 2026-09-02): the rest of this phase runs as one round** —
#1364 (the timer, from Phase 3), #1367 and #1499 share the same strip of the
Focus Player and two of them add schema, so they get one Claude Design pass
and one Tier 3 spec, `specs/practice-instruments.md`. Implementation still
lands as separate PRs, smallest first.

**The round shipped 2026-09-03** in six PRs: the spec, the design pass and the
session timer (#1502), the tempo constants it needed (#1504), the pass counter
(#1505, #1506), and the honest click (#1509, #1511). #1499 also moved `Metre`
onto the item, which retired `ChordChart.metre`. Left tracked rather than
built: #1510 (a quaver-metre piece still shows its declared tempo as a
crotchet), #1512 (sheets hand-mirroring core-validated ranges), #1513 (the
screen sleeps mid-practice), #1507 and #1508 (pass-counter at-target state and
haptics).
