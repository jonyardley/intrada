...
3. **Self-review**: per CLAUDE.md §Workflow/Always(4): for non-trivial work, run the code-reviewer subagent via the `requesting-code-review` skill (include "comment-policy violations are Blockers, not Nits"); for small fixes use `/review`. Apply blockers inline.
...
6. **Status**: if this PR changes what's in flight (lands, opens, or defers work), update `docs/status.md` on the same branch — move the item between sections and bump the `Last updated` line. Skip only for changes with no scope effect (typos, dep bumps).
