...
1. **Gates**, through the `test-runner` agent: `just check`, plus `just ios-fmt-check` and `just ios-test` when files under `ios/` changed, plus `just ios-test-ui-class <Class>` for each UI test class the diff adds or edits. Not the full tier: CI runs every UI test on the draft, six at a time, and is the first full UI run (#2114). Stop at the first failure and report its actual output.
