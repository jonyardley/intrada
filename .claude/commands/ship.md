...
2. **Self-review**, before the push: the `reviewer` agent over the local diff, pinned to Opus 5 high and briefed with the worktree and the issue. Check `git diff --stat origin/main...HEAD | grep -E 'intrada-ffi|ios/generated|LibraryStore.swift|domain/session.rs'`; a hit spawns it with `model: fable` instead. Fix blockers inline. A fix touching `ios/` reruns `just ios-test` plus `just ios-test-ui-class <Class>` for the class the fix touched, not `ios-test-full` again (#1884); a fix with no UI test to name just reruns `just ios-test`.
