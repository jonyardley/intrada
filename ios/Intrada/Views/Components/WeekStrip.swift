    ...
    Button(action: onTap) {
      VStack(spacing: 5) {
        Text(weekdayInitial)
          ...
          .foregroundStyle(isSelected || isToday ? IntradaColor.accentText : IntradaColor.inkFaint)
        Text(dayNumber)
          ...
          .background(isSelected ? IntradaColor.accentText : .clear, in: Circle())
          .overlay(
            Circle().strokeBorder(
              IntradaColor.accentText, lineWidth: isToday && !isSelected ? 1.5 : 0)
          )
        Circle()
          .fill(hasPractice ? IntradaColor.accentGraphic : .clear)
