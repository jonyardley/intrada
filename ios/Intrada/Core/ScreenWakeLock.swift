...
/// flag injected so the release path is testable. `@unchecked Sendable` is the
/// price of being an `EnvironmentKey.defaultValue`: every member is `@MainActor`,
/// so `isHeld` is only ever touched there.
